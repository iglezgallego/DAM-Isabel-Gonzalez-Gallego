...
<html lang="en">
  ...
  <body>
    ...
    <header class="navbar navbar-dark sticky-top bg-dark flex-md-nowrap p-0 shadow">
        ...
        <p style="color:#DBD5DC; padding-left:10px; font-size:20px;">Panel de control - Insertar nuevo reto</p>
    ...
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
     ...
     <!-- Pagina principal del panel de control -->
      <div class="table-responsive">
        ...
        <form class="needs-validation" novalidate method="POST" action="insertarreto.php">
          <!-- input para saber que tabla estamos pasando -->
            <?php
                $consulta = "SHOW COLUMNS FROM retos";
                $resultado = $conexion->query($consulta);

                while ($fila = $resultado->fetch_assoc()) {
                    if ($fila['Field'] == "Identificador" || $fila['Field'] == "idhijo") {
                        continue;
                    }
                    echo '<input type="hidden" class="form-control" id="address2" name="idpadre" value="' . $_GET['idpadre'] . '">';
